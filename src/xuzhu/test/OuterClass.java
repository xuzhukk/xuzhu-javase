 ...
 * 
 * 从上面代码中看好像name参数应该是被内部类直接调用？其实不然，在java编译之后实际的操作如下： [java] view plain copy
 * public class OuterClass$InnerClass { public InnerClass(String name,String
 * age){ this.InnerClass$name = name; this.InnerClass$age = age; }
 * 
 * 
 * public void display(){ System.out.println(this.InnerClass$name + "----" +
 * this.InnerClass$age ); } } 所以从上面代码来看，内部类并不是直接调用方法传递的参数，而是利用自身的构造器对传入的参数进行备份，
